...
- picker selected=初始位置语义，change 回调禁回写绑定字段（草稿与初始绑定分离）；$idx/$item 仅 for 循环内有效：静态元素 onclick 传 $idx 得 undefined（用独立无参 handler）；for 内传参必须 $idx，handler 按 this.rows[idx] 取条目
